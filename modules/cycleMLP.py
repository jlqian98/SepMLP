"""
CYCLEMLP: A MLP-LIKE ARCHITECTURE FOR DENSE PREDICTION (Chen et. al )
    ...
    def gen_offset(self):
        ...
        for i in range(self.in_channels):
            if self.kernel_size[0] == 1:
                ...
                offset[0, 2 * i + 1, 0,
                       0] = (i + start_idx) % self.kernel_size[1] - (self.kernel_size[1] // 2)
            else:
                offset[0, 2 * i + 0, 0,
                       0] = (i + start_idx) % self.kernel_size[0] - (self.kernel_size[0] // 2)
...
class CycleBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = mlp_fn(dim, qkv_bias=qkv_bias,
                           qk_scale=None, attn_drop=attn_drop)
...
def cycle_blocks(dim, index, layers, mlp_ratio=4., qkv_bias=False, qk_scale=None, attn_drop=0.,
